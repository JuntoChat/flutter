...
class WebDriverService extends DriverService {
  ...
  Future<int> startTest(
    ...
    try {
      ...
      throwToolExit('Unable to start a WebDriver session for web testing.\n'
    ...
    final processStream = _processUtils.stream(<String>[
      _dartSdkPath,
      ...arguments,
      testFile,
      '-rexpanded',
    ...
    processStream.then((_) => isFinished = true);

    while (!isFinished) {
      await Future.delayed(Duration(seconds: 5));
      webDriver.logs
          .get(async_io.LogType.browser)
          .listen((async_io.LogEntry logEntry) {
        print(logEntry);
      });
    }

    final int result = await processStream;
...
Map<String, dynamic> getDesiredCapabilities(Browser browser, bool headless,
  ...
  throw UnsupportedError(
      'Browser $browser not supported.'); // dead code; remove with null safety migration
}
